...
<div id="content">
    ...
    <div id="breadcrumb">
        ...
        <a href="/apartment/findAll" class="current">公寓信息</a>
    </div>
    <div>
        <button class="btn" onclick="location.href='/apartment/add'">添加公寓</button>
    </div>
                    ...
                    <div class="widget-content nopadding">
                        <table class="table table-bordered table-striped with-check">
                            ...
                            <c:forEach items="${map}" var="mp">
                                <tr>
                                    <td><input  name="ids" type="checkbox" value="${mp.key.apartmentid}"></td>
                                    <td>${mp.key.apartmentid}</td>
                                    <td>${mp.key.fee}</td>
                                    <td>${mp.key.maxoccupancy}</td>
                                    <td>${mp.value}</td>
                                    <td><button type="button" class="btn " onclick="location.href='/apartment/apartmentDetail?apId=${mp.key.apartmentid}'"><i class="icon-eye-open"></i>查看详情</button>
                                        <button type="button" class="btn btn-inverse" onclick="location.href='/apartment/updateap?apId=${mp.key.apartmentid}'"><i class="icon-refresh icon-white"></i>更改</button>
                                    <c:if test="${mp.key.dorms.size()==0}"><button type="button" class="btn btn-danger" onclick="location.href='/apartment/delete?apId=${mp.key.apartmentid}'"  ><i class="icon-remove icon-white"></i>删除</button></c:if>
                                    <c:if test="${mp.key.dorms.size()!=0}"><button type="button" class="btn btn-danger" onclick="location.href='/apartment/delete?apId=${mp.key.apartmentid}'" disabled="disabled"><i class="icon-remove icon-white"></i>删除</button></c:if></td>
                                </tr>
                            </c:forEach>
                            </tbody>
                        </table>
...
<script>
    function deleteDorm() {
        ...
        if (flag) {
            ...
            alert(array);
            location.href = "/apartment/deleteList?string=" + array;
        }else {
            alert("请至少选择一个");
        }
    }
